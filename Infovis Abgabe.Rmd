...
college_major %>% 
  select(Men, Women, Full_time, Part_time) %>% 
  gather(gender, variable, value, -year) %>%
  summarize(subtotal = sum(value)) %>% 
  mutate(Full_time = fct_reorder(Full_time, subtotal)) %>% 
  mutate(Part_time = fct_reorder(Part_time, subtotal)) %>% 
  mutate(type_of_work = case_when(
    str_detect(variable, "Part") ~ "part time",
    str_detect(variable, "Full") ~ "full time")) %>%  
  mutate(value = value / 100) %>% 
  ggplot() +
  aes(x = gender, y = subtotal, fill = type_of_work) + 
...
# Die beiden in ein Diagramm zur besseren Vergleichbarkeit
...
jobs_gender %>% 
  ...
  total_lohn_female <- c("total_earnings_female")
  total_lohn_male <- c("total_earnings_male")
 
  
   jobs_gender %>%   
filter(year == 2013, major_category != "Healthcare Practitioners and Technical") %>% 
  select(-starts_with("total")) %>% 
  gather(gender, variable, value, -year, -major_category) %>%
    group_by (major_category) %>% 
    summarise(total = mean(total_lohn_female),
              total = mean(total_earnings_male)) %>% 
  mutate(gender = case_when(
    str_detect(variable, "female") ~ "female",
    str_detect(variable, "male") ~ "male"),
    gehalt = case_when(
    str_detect(variable, "lohn") ~ "lohn female",
    str_detect(variable, "earnings") ~ "earnings male")) %>%  
  mutate(value = value / 100) %>% 
  ggplot() +
  aes(x = major_category, y = value, fill = gehalt) + 
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
    coord_flip() +
  scale_fill_manual(values = c(rwthcolors$turquois, rwthcolors$orange)) 
...
jobs_gender %>% 
  ...
  gather(gender, variable, -year, -major_category, -total_earnings) %>% 
  group_by (major_category) %>% 
  mutate(gender = case_when(
    str_detect(variable, "female") ~ "female",
    str_detect(variable, "male") ~ "male")) 
