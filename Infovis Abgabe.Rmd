...
jobs_gender %>% select(major_category, minor_category, workers_male, workers_female) %>% 
  ...
  ggplot() +
  aes(x = major_category, y = total, fill = gender) + geom_col() + coord_flip() +
  scale_fill_manual(name = "Legend", labels = c("Female", "Male"), values = c(rwthcolors$red, rwthcolors$blue)) +
...
college_major %>% 
  ...
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  scale_fill_manual(name = "Legend", labels = c("Full Time", "Part Time", "Unemployed"), values = c(rwthcolors$turquois, rwthcolors$orange, rwthcolors$petrol)) +
